...
export class FaustBrowserMcpServer {
  private runtime: any; // Reference to the Faust browser runtime
  private docStore: FaustDocStore | null = null;
   ...
   */
  async initDocStore(docIndexUrl: string): Promise<void> {
    try {
  ...
  private async handleToolCall(params: any): Promise<any> {
    ...
    try {
      ...
      if (name === 'search_faust_lib') {
        if (!this.docStore) throw new Error('Documentation not available');
   ...
   */
  private async callRuntimeTool(name: string, args: any): Promise<any> {
    const a = args || {};
    switch (name) {
      case 'compile_and_start':
        return this.runtime.compile_and_start(
          a.faust_code, a.name, a.latency_hint,
          a.input_source, a.input_freq, a.input_file,
          a.hide_meters, a.double_precision
        );
      case 'compile':
      ...
      case 'get_audio_metrics':
        return this.runtime.get_audio_metrics(
          a.include_scope, a.include_spectrum, a.per_channel,
          a.fft_size, a.smoothing, a.min_db, a.max_db,
          a.edge_threshold, a.log_bins
        );
